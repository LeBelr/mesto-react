export default function PopupWithForm({title, name, btnText, children, isOpen, onClose}) {
    ...
    <div className={`popup popup_type_${name} ${isOpen ? 'popup_opened' : ''}`}>
      <div className="popup__content">
        ...
        <form className="popup__form" name={`${name}Form`}>
          {children}
          <button type="submit" className={`popup__submit popup__submit_type_${name}`}>{btnText}</button>
        </form>
      </div>
    </div>
  )
